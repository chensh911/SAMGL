...
def get_ogb_evaluator(dataset):
    def get_metric(y_true, y_pred):
        ...
        return {'MAE': MAE, 'SRC': SRC, 'nMSE': nMSE}
    return lambda preds, labels: get_metric(labels, preds)


def load_dataset(args):
    g, target_node_type, feature_node_types, (train_nid, val_nid, test_nid), \
    ...
    init_labels = g.nodes[target_node_type].data["label"]
    n_classes = 1
    ...
    new_g.nodes['F'].data['F'] = g.nodes['frame'].data['feat']
    new_g.nodes['U'].data['U'] = g.nodes['user'].data['feat']
    new_g.nodes['T'].data['T'] = g.nodes['title'].data['feat']
    new_g.nodes['V'].data['V'] = g.nodes['video'].data['feat']

    # FV, TV, UV, VF, VT, VU = adjs
    ...
    return new_g, init_labels, new_g.num_nodes('V'), n_classes, train_nid, val_nid, test_nid, evaluator
