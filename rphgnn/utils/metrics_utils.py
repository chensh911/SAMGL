...
class MAE(LogitsBasedMetric):
    ...
    def update(self, logits: torch.Tensor, target: torch.Tensor):
        absolute_errors = torch.abs(logits - target)
        self.sum_absolute_error += absolute_errors.sum()
        self.total += target.size(0)

    def compute(self):
        return self.sum_absolute_error / self.total.float()


class SRC(LogitsBasedMetric):
    def __init__(self):
        super().__init__()
        self.add_state("src_sum",default=torch.tensor(0.0), dist_reduce_fx="sum")
        self.add_state("total", default=torch.tensor(0), dist_reduce_fx="sum")

    def update(self, logits: torch.Tensor, target: torch.Tensor):
        batch_src_sum, _ = spearmanr(logits.cpu(), target.cpu())
        self.src_sum += torch.tensor(batch_src_sum.sum()).to(logits.device)
        self.total += 1

    def compute(self):
        return self.src_sum / self.total.float()

class nMSE(LogitsBasedMetric):
    def __init__(self):
        super().__init__()
        self.add_state("nmse_sum",default=torch.tensor(0.0), dist_reduce_fx="sum")
        self.add_state("total", default=torch.tensor(0), dist_reduce_fx="sum")

    def update(self, logits: torch.Tensor, target: torch.Tensor):
        batch_nmse = torch.mean((logits - target) ** 2) / (target.std() ** 2)
        self.nmse_sum += batch_nmse.to(logits.device)
        self.total += 1

    def compute(self):
        return self.nmse_sum / self.total.float()