...
def rphgnn_propagate_and_collect(g, k, inner_k, alpha, target_node_type, use_input_features, squash_strategy, train_label_feat, norm, squash_even_odd, collect_even_odd, squash_self=False, target_feat_random_project_size=None, add_self_group=False, set_zero_types=None):
    ...
    with torch.no_grad():
        ...
        with g.local_scope():
            ...
            if set_zero_types != None:
                for set_zero_type in set_zero_types:
                    g.nodes[set_zero_type].data["feat"] = torch.zeros([g.num_nodes(set_zero_type), 64])
