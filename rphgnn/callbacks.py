...
class EarlyStoppingCallback(Callback):
    ...
    def on_epoch_end(self, epoch, logs):
        ...
        if self.strategy == "common":
            ...
            reset_patience_counter = val_score < self.min_val_score
            print(f"epoch {epoch}, reset_patience_counter {reset_patience_counter}")
        else:
            raise ValueError("Unknown strategy: {}".format(self.strategy))
        print(f"validation_freq {self.validation_freq}, patience_counter {self.patience_counter}")
        ...
        if not stop:
            if self.strategy == "common":
                ...
                should_update = val_score < self.min_val_score
            else:
                raise ValueError("Unknown strategy: {}".format(self.strategy))
