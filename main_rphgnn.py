...
parser.add_argument("--r", type=int, required=False, default=None)
...
feat_target_h_list_list =[torch.tensor(np.mean([feat_target_h_list[retrieval_idx[idx][:retrieval_num]] for idx in range(len(retrieval_idx))], axis=1)) for feat_target_h_list in feat_target_h_list_list]
feat_target_h_list_list = nested_map(feat_target_h_list_list, lambda x: x.to(target_h_dtype).to(pre_device))
target_h_list_list += feat_target_h_list_list
...
model = RpHGNNEncoder(
    ...
    [hidden_size] * (num_layers_list[2] - 1) + [1],
    merge_mode,
